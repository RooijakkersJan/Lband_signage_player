...
//        Toast.makeText(context, message, Toast.LENGTH_SHORT).show();
	...
	public static String getApplicationDirectory(Context context){
		...
		if (Utilities.isVersionLowerThanLollipop()){
			...
			File[] pathsss = context.getExternalFilesDirs(null);
			...
			if (externalLocations.size() > 1){
				...
				pathToUsb = context.getApplicationInfo().dataDir;
			}
			...
			long bytesAvailable;
			long megAvailable;
			File[] pathsss =  ContextCompat.getExternalFilesDirs(context.getApplicationContext(), null);
	...
	public static String getExistingFilePath(Context context, Songs songs){

		String applicationDirectory = Utilities.getApplicationDirectory(context);

		String fileURL = songs.getTitle_Url();

		final String fileName = Utilities.removeSpecialCharacterFromFileName(songs.getTitle());

		final String filePath = applicationDirectory + File.separator+ songs.getTitle_Id() + Constants.TAG_FILE_EXTENSION_MP3;
		String filePath1 = Utilities.getGlobalStorage(context) + File.separator + songs.getTitle_Id() + Constants.TAG_FILE_EXTENSION_MP3;
	...
	public static String getGlobalStorage(Context context){

		String pathToUsb = "";

		File gblStorage = null;
		...
		if (gblStorage != null){
			return gblStorage.getAbsolutePath();
		} else {
			return "";
		}
...
//		return "23b8f0b8a6de10fevideo"; // TODO Comment this line.
		return Settings.Secure.getString(context.getContentResolver(),
				Settings.Secure.ANDROID_ID) + Constants.VIDEO_TAG;
	}
	...
	public static String currentTimeexit(){
		Calendar calendar;
		calendar =Calendar.getInstance();
	...
	public static String removeFileExtension(String fileName, Context context){
		if (fileName.endsWith(context.getString(R.string.mp4_file_extension)))
			return fileName.substring(0,fileName.length() - context.getString(R.string.mp4_file_extension).length());
		else
			return fileName;
	}
